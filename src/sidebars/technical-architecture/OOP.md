...
| ------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Encapsulation**         | **Đóng gói**       | Bảo vệ dữ liệu, hạn chế truy cập trái phép, chỉ cho phép truy cập những thuộc tính thông qua những phương thức được định nghĩa công khai.           | Ví dụ: Lớp `BankAccount` ẩn chi tiết số dư tài khoản và chỉ cung cấp các phương thức công khai như `deposit()` và `withdraw()` để thao tác với số dư.                      |
| **Abstraction**           | **Trừu tượng hóa** | Ẩn đi chi tiết không cần thiết, chỉ hiển thị những gì quan trọng.                                                                                   | Ví dụ: Lớp `Vehicle` có thể có các phương thức `start()` và `stop()`, nhưng chi tiết về cách mỗi loại xe hoạt động có thể được ẩn đi trong các lớp con như `Car`, `Truck`. |
