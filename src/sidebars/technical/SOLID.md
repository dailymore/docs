...
| **O**          | **Open/Closed Principle (OCP)**           | Các lớp phải mở để mở rộng, nhưng đóng để sửa đổi. Bạn có thể mở rộng chức năng mà không cần sửa đổi mã nguồn hiện tại.                   | Ví dụ: Một lớp `Shape` có phương thức `draw()`. Khi cần thêm các hình dạng mới như `Circle`, `Rectangle`, thay vì thay đổi lớp `Shape`, ta tạo thêm các lớp con như `Circle` và `Rectangle` mở rộng từ `Shape` để vẽ hình mới mà không thay đổi lớp gốc.                                                                                                                                                   |
